...
resource_contrast <- capital %>% filter(model == "RC2_CONTRAST") %>% slice(1)
...
table2 <- bind_rows(
  ...
  tibble(Analysis="Resource-composition contrast", Outcome="Disease-specific TCM treatment use", Measure="TCM bed minus comprehensive-hospital-bed coefficient", Estimate=fmt_ci(resource_contrast$effect,resource_contrast$ci_lower,resource_contrast$ci_upper), `Conventional P value`=fmt_p(resource_contrast$conventional_pvalue), `Small-cluster inference`=paste0("CR2 ",fmt_ci(resource_contrast$effect,resource_contrast$cr2_ci_lower,resource_contrast$cr2_ci_upper),"; P = ",fmt_p(resource_contrast$cr2_pvalue))),
...
resource_table <- bind_rows(
  contextual %>% filter(model %in% c("C1","C3","C4","C5"), term %in% c("z_tcm_beds","z_comprehensive_beds","z_total_hospital_beds")) %>% transmute(Model=model_label,Measure=term_label,`Conventional estimate`=fmt_ci(effect,ci_lower,ci_upper),`Conventional P value`=fmt_p(cluster_pvalue),`CR2 estimate or P value`=paste0(fmt_ci(effect,cr2_ci_lower,cr2_ci_upper),"; P = ",fmt_p(cr2_pvalue))),
  ...
  joint %>% filter(model == "J4", term %in% c("z_lag_tcm_beds","z_lag_tcm_physicians","z_comprehensive_beds")) %>% transmute(Model="Lagged contextual resource model",Measure=term_label,`Conventional estimate`=fmt_ci(effect,ci_lower,ci_upper),`Conventional P value`=fmt_p(cluster_pvalue),`CR2 estimate or P value`=paste0(fmt_ci(effect,cr2_ci_lower,cr2_ci_upper),"; P = ",fmt_p(cr2_pvalue))),
  capital %>% filter(model %in% c("RC0_CONTRAST","RC1_CONTRAST","RC2_CONTRAST","CL1","CL2_CONTRAST")) %>% transmute(Model=model_label,Measure=exposure,`Conventional estimate`=fmt_ci(effect,ci_lower,ci_upper),`Conventional P value`=fmt_p(conventional_pvalue),`CR2 estimate or P value`=paste0(fmt_ci(effect,cr2_ci_lower,cr2_ci_upper),"; P = ",fmt_p(cr2_pvalue)))
)
